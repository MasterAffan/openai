...
class SupabaseService:
    ...
    def get_user_id_from_request(self, request) -> Optional[str]:
        """Extract Bearer token from Authorization header and return user id."""
        # Support both FastAPI and generic request objects
        auth_header = None
        ...
        # Fallback for BlackSheep style
        if not auth_header and hasattr(request, 'get_first_header'):
            header_bytes = request.get_first_header(b"authorization")
            if header_bytes:
                auth_header = header_bytes.decode()
